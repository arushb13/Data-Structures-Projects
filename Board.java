...
public class Board {
    ...
    public void updateOpenSpaces() {
        ...
        while (i < gameBoard.length){
            ...
            while (j < gameBoard[i].length){
                if (gameBoard[i][j] == 0){
                    BoardSpot neo = new BoardSpot(i,j);
                    openSpaces.add(neo);
                }
            j++;
            }
    ...
    public void addRandomTile() {
        int p = StdRandom.uniform(openSpaces.size());
        double val = StdRandom.uniform();
        BoardSpot randomTile = openSpaces.get(p);
        int tile = 0;
        if (val < 0.1){
            tile = 4;
        }
        ...
        gameBoard[randomTile.getRow()][randomTile.getCol()] = tile;
    }
    ...
    public void flipRows() {
        ...
        while (i < gameBoard.length){
            int j = 0;
            while (j < gameBoard[i].length/2){
                int x;
                x = gameBoard[i][j];
                gameBoard[i][j] = gameBoard[i][(gameBoard[i].length - 1) - j];
                gameBoard[i][(gameBoard[i].length - 1) - j] = x;
            j++;
            }
        i++;
        }
