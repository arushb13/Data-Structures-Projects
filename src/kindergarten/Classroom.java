...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        while (i < numStu){
            ...
            SNode p = new SNode(stu, studentsInLine);
            studentsInLine = p;
            i++;
        }
    ...
    public void seatStudents () {
        ...
        while (i < seatingLocation.length){
            int j = 0;
            while (j < seatingLocation[i].length){
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        if (rand == size - 1) {
            ...
            selectStu = musicalChairs.getStudent();
            musicalChairs = current;
            musicalChairs.setNext(current.getNext().getNext());
        }
        else if (rand == 0) {
            // first chair
            SNode removeNode = musicalChairs.getNext();
            selectStu = removeNode.getStudent();
            musicalChairs.setNext(removeNode.getNext());
        }
        else {
            // chair in middle
            int i = 0;
            ...
            SNode removeNode = current.getNext();
            selectStu = removeNode.getStudent();
            current.setNext(removeNode.getNext());
        }
        insertByHeight(selectStu);
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        if (studentsInLine == null || studentToInsert.getHeight() < studentsInLine.getStudent().getHeight()) {
            ...
            for (SNode current = studentsInLine.getNext(); current != null; current = current.getNext()) {
                if (studentToInsert.getHeight() < current.getStudent().getHeight()) {
    ...
    public void eliminateLosingStudents() {
        ...
        int d = 1;
        SNode p = musicalChairs.getNext();
        while (p != musicalChairs){
            d++;
            p = p.getNext();
        }
        int i = d;
        while (i > 1) {
            moveStudentFromChairsToLine(d);
            i--;
        }
    ...
    public void seatMusicalChairsWinner () {
        ...
        int i = 0;
        while (i < studentsSitting.length){
            int j = 0;
            while (j < studentsSitting[i].length){
                if (seatingLocation[i][j] == true){
                    studentsSitting[i][j] = w;
